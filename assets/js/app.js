...
function viewHome(){ 
  ...
  <section class="section"><div class="container"><h2 class="kicker">Polecane</h2>
    <div class="gallery">
      <img src="assets/img/photos/ph_1.svg" alt="Margherita"/><img src="assets/img/photos/ph_2.svg" alt="Diavola"/><img src="assets/img/photos/ph_3.svg" alt="Quattro"/>
      <img src="assets/img/photos/ph_4.svg" alt="Pistacchio"/><img src="assets/img/photos/ph_5.svg" alt="Tartufo"/><img src="assets/img/photos/ph_6.svg" alt="Panna Cotta"/>
    </div></div></section>`;
  return el;
}
function viewMenu(){ 
  const el = document.createElement('div'); el.className='view enter';
  const cards = [{"id": "margherita", "n": "Margherita", "d": "San Marzano, fior di latte, bazylia", "p": 28, "img": "assets/img/photos/ph_1.svg"}, {"id": "diavola", "n": "Diavola", "d": "Spianata piccante, mi\u00f3d peperoncino", "p": 39, "img": "assets/img/photos/ph_2.svg"}, {"id": "quattro", "n": "Quattro Formaggi", "d": "Fior di latte, gorgonzola, parmezan, pecorino", "p": 41, "img": "assets/img/photos/ph_3.svg"}, {"id": "pistacchio", "n": "Pistacchio e Mortadella", "d": "Krem pistacjowy, mortadella, burrata", "p": 46, "img": "assets/img/photos/ph_4.svg"}, {"id": "tartufo", "n": "Tartufo", "d": "Krem truflowy, grzyby, rukola", "p": 44, "img": "assets/img/photos/ph_5.svg"}, {"id": "panna", "n": "Panna Cotta", "d": "Z sosem malinowym", "p": 17, "img": "assets/img/photos/ph_6.svg"}].map(p=>`
...
function viewPrivacy(){
  ...
  el.innerHTML = `<section class="section"><div class="container card">
    ...
    <p>Administrator: Pizza Italiana Napoletana. Dane przetwarzamy w celu realizacji zamówień i obowiązków prawnych. Prawa: dostęp, sprostowanie, usunięcie, ograniczenie, sprzeciw, przenoszenie, skarga do PUODO.</p>
    <p>Cookies: techniczne, nawigacyjne i koszykowe. Dostosuj treść pod swoje realne procesy.</p>
  </div></section>`;
  return el;
}
function viewTerms(){
  ...
  el.innerHTML = `<section class="section"><div class="container card">
    <h2>Regulamin</h2>
    <ol>
      <li>Zamówienia wyłącznie online z płatnością Stripe (także przy odbiorze osobistym).</li>
      <li>Ceny w PLN z VAT.</li>
      <li>Dostawa w obrębie strefy; opłata doliczana w koszyku.</li>
      <li>Reklamacje prosimy zgłaszać telefonicznie lub e‑mailowo.</li>
    </ol>
  </div></section>`;
  return el;
}
function viewSuccess(){
  const el=document.createElement('div'); el.className='view enter';
  el.innerHTML = `<section class="section"><div class="container card"><h2>Dziękujemy!</h2><p>Płatność przyjęta. Szykujemy Twoje zamówienie.</p><button class="cta" onclick="go('/')">Wróć</button></div></section>`;
  return el;
}
function viewCancel(){
  const el=document.createElement('div'); el.className='view enter';
  el.innerHTML = `<section class="section"><div class="container card"><h2>Płatność anulowana</h2><p>Możesz spróbować ponownie.</p><button class="cta" onclick="go('/contact')">Wróć do zamówienia</button></div></section>`;
  return el;
}

// Cart
function addToCart(p){ const f=CART.find(x=>x.id===p.id); if(f) f.qty++; else CART.push({id:p.id,name:p.n,price:p.p,qty:1}); updateCart(); }
function updateCart(){ save(); const cnt=CART.reduce((a,b)=>a+b.qty,0); const cs=document.querySelector('.cart-fab .count'); if(cs) cs.textContent=cnt; const dm=document.getElementById('deliveryMethod'); if(dm) setTotals(totals(dm.value)); }
...
function renderDrawer(){ const box=$('#drawer-items'); box.innerHTML=''; CART.forEach(it=>{ const row=document.createElement('div'); row.className='item'; row.innerHTML=`<div><strong>${it.name}</strong><div class="kicker">${fmt(it.price)} × ${it.qty}</div></div><div class="qty"><button data-a="dec">−</button><span>${it.qty}</span><button data-a="inc">+</button><button data-a="rm" style="margin-left:6px">✕</button></div>`; row.querySelector('[data-a=inc]').onclick=()=>{it.qty++; updateCart(); renderDrawer();}; row.querySelector('[data-a=dec]').onclick=()=>{it.qty=Math.max(1,it.qty-1); updateCart(); renderDrawer();}; row.querySelector('[data-a=rm]').onclick=()=>{ CART = CART.filter(x=>x.id!==it.id); updateCart(); renderDrawer(); }; box.appendChild(row); }); const dmVal=document.getElementById('deliveryMethod')?.value||'pickup'; const t=totals(dmVal); $('#drawer-subtotal').textContent=fmt(t.sub); $('#drawer-delivery').textContent=fmt(t.del); $('#drawer-total').textContent=fmt(t.total); $('.cart-fab .count').textContent=CART.reduce((a,b)=>a+b.qty,0); }

// Router mount
function render(){ const path=location.hash.replace('#','')||'/'; const view=(routes[path]||viewHome)(); const root=document.getElementById('view'); root.innerHTML=''; root.appendChild(view); }
document.addEventListener('DOMContentLoaded',()=>{ $$('#nav a').forEach(a=>a.addEventListener('click',e=>{e.preventDefault();go(a.getAttribute('href').replace('#',''));})); document.querySelector('.cart-fab').addEventListener('click',openDrawer); document.getElementById('closeDrawer').addEventListener('click',closeDrawer); updateCart(); render(); });
